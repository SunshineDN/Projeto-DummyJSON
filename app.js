...
window.addEventListener("DOMContentLoaded", async e => {
    ...
    let profilePageUser;
    let user = await fetch("https://dummyjson.com/users/" + userId)
    .then(res => res.json())
    .then(res => profilePageUser = res);
    window.localStorage.setItem("user", JSON.stringify(profilePageUser))

    if (user == undefined || ("message" in user)) {
        return
    }
    
    headerContainer.innerHTML += writeUser(user)
...
function writeUser(e) {
    return `<div class="user_area">
                <p class="user_name user_link">${e.firstName} ${e.lastName}</p>
                <a href="#"><img class="user_pic user_link" src=${e.image} alt="Foto de ${e.firstName} ${e.lastName}"></a>
            </div>`
}
...
window.addEventListener("DOMContentLoaded", async e => {
    ...
    let commentJSON = await fetch("https://dummyjson.com/comments/")
    .then(res => res.json())
    .then(({comments}) => comments);

    if (commentJSON == undefined) {
        main.innerHTML = `
        <h1 class="post_error">ERROR LOADING POSTS</h1>`
        return
    }

    commentJSON = commentJSON.sort(() => Math.random() - 0.5);
    let postJSON = [], userPostJSON = [], userCommentIMGJSON = [];
    console.log(commentJSON[0], commentJSON[1])
    
    for(i = 0; i < commentJSON.length; i++) {
        postJSON[i] = await fetch('https://dummyjson.com/posts/' + commentJSON[i].postId)
        .then(res => res.json());
        console.log("PostJson: ", postJSON[i])
        
        userCommentIMGJSON[i] = await fetch('https://dummyjson.com/users/' + commentJSON[i].user.id)
        .then(res => res.json());
    
        userPostJSON[i] = await fetch('https://dummyjson.com/users/' + postJSON[i].userId)
        .then(res => res.json());

        if (postJSON[i] == undefined || ("message" in postJSON[i])) {
            main.innerHTML = `
            <h1 class="post_error">ERROR LOADING POSTS</h1>`
            return
        }
    ...
    for(i = 0; i < commentJSON.length; i++) {
        if (i == 0) {
            main.innerHTML = `<img src="BMO.gif" alt="BMO Gif" id="bmo">`
        }
        main.innerHTML += writePost(postJSON[i], userPostJSON[i], userCommentIMGJSON[i], commentJSON[i])
    }

    let likes = document.querySelectorAll(".likes_svg");
    likes.forEach(element => {
        element.addEventListener('click', e => {
            element.classList.toggle("likes_svg_white")
        })
    })
})

function writePost(post, userPost, userCommentIMG, comment) {
    return `
    <div class="container">
            <div class="account">
                <img src="${userPost.image}" alt="Picture do usuário ${userPost.id}" class="pic_acc">
                <div class="name_username">
                    <p class="post_username">${userPost.firstName} ${userPost.lastName}</p>
                    <p class="post_sec_username">#${userPost.username}</p>
                </div>
            </div>
            <div class="post">
                <h1 class="post_title">˹${post.title}˼</h1>
                <p class="post_body">${userPost.body}</p>
                <div class="post_footer">
                    <span class="tags">TAGS: ${post.tags.join(", ")}</span>
                    <div class="likes post${post.id}">
                        <div alt="Like svg">
                            <svg class="likes_svg likes_svg_white" version="1.1" viewBox="0 0 700 700" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <defs>
                                    ...
                                    <symbol id="g" overflow="visible">
                                        <path d="m2.5781-20.406h5.25v20.406h-5.25z"></path>
                                    </symbol>
                                ...
                                <g>
                                    <path d="m226.8 448v-212.8c0-6.1602-7.8398-11.199-14-11.199h-78.398c-6.1641 0-14.004 5.0391-14.004 11.199v212.8c0 6.1602 7.8398 11.199 14 11.199h78.398c6.1641 0 14.004-5.0391 14.004-11.199zm-56-11.199c-7.8398 0-14-6.1602-14-14 0-7.8398 6.1602-14 14-14 7.8398 0 14 6.1602 14 14 0 7.8398-6.1602 14-14 14z"></path>
                                    <path d="m462 453.6h43.68c13.441 0 25.199-9.5195 26.32-22.961 1.6797-15.121-10.641-27.441-25.199-27.441h-1.6797l0.55859-6.1602c1.1211 0 2.2383 0.55859 3.3594 0.55859h10.078c13.441 0 25.762-8.9609 28.559-22.398 3.9219-17.922-10.078-33.602-27.441-33.602h-7.2812l0.55859-5.6016h2.2383l19.051 0.003906c14 0 25.199-11.199 25.199-25.199v-5.0391c0-14-11.199-25.199-25.199-25.199h-12.32l0.55859-5.6016h7.8398c17.359 0 32.48-12.32 34.719-29.121 2.8008-20.16-13.441-38.078-33.602-38.078h-145.04v-0.55859c1.6797-2.8008 16.801-25.762 16.801-77.84 0 0 0-50.398-33.602-61.602-0.55859 0-16.801 0.55859-22.398 28-5.6016 28-2.8008 95.199-103.6 151.2v184.8c1.1211 1.1211 25.762 22.398 58.801 22.398h159.04z"></path>
                                </g>
                            </svg>
                        </div>
                        <p class="qtde_likes">${post.reactions}</p>
                    </div>
                </div>
            </div>
            <hr class="divisor">
            <div class="comment">
                <div class="comment_header">
                    <img src="${userCommentIMG.image}" alt="User ID ${comment.user.id}" class="pic_acc_comment">
                    <p class="sec_username_comment">${comment.user.username}</p>
                </div>
